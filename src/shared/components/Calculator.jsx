...
function Calculator({ isVisible, onClose }) {
  ...
  const fetchCategoriesByCard = async (cardValue) => {
    try {
      ...
      // 使用真實的 API（GET 請求）
      const response = await fetch(
        `https://rewardia.net/get-categories-by-card/?card_select=${encodeURIComponent(
          cardValue
        )}`,
        {
          method: "GET",
  ...
  const fetchScopesByCategory = async (categoryValue) => {
    try {
      ...
      // 使用真實的 API（GET 請求）
      const response = await fetch(
        `https://rewardia.net/get-scopes-by-category/?card_select=${encodeURIComponent(
          formData.card
        )}&category_select=${encodeURIComponent(categoryValue)}`,
        {
          method: "GET",
          headers: {
            "X-Requested-With": "XMLHttpRequest",
      ...
      <div style={{ padding: "16px", backgroundColor: "#f8f9fa" }}>
        ...
        <div style={{ marginBottom: "16px", position: "relative" }}>
          ...
          <motion.button
            type="button"
            onClick={() =>
              formData.bank && setShowCardDropdown(!showCardDropdown)
            }
          ...
          >
            ...
            >
              {formData.card
                ? options.cards.find((c) => c.value === formData.card)?.text ||
                  formData.card
                : !formData.bank
                ? "請先選擇銀行"
                : "請選擇卡片"}
        ...
        <div style={{ marginBottom: "16px", position: "relative" }}>
          ...
          <motion.button
            type="button"
            onClick={() =>
              formData.card && setShowCategoryDropdown(!showCategoryDropdown)
            }
          ...
          >
            ...
            >
              {formData.category
                ? options.categories.find((c) => c.value === formData.category)
              ...
              >
                {options.categories.map((category, index) => (
                  <motion.div
                    ...
                    whileHover={{ backgroundColor: "#f3f4f6" }}
                    onClick={() =>
                      handleCategorySelect(category.value, category.text)
                    }
                    style={{
                      padding: "8px 12px",
                      cursor: "pointer",
                      fontSize: "14px",
                      borderBottom:
                        index < options.categories.length - 1
                          ? "1px solid #e5e7eb"
                          : "none",
                    }}
                  >
        ...
        <div style={{ marginBottom: "16px", position: "relative" }}>
          ...
          <motion.button
            type="button"
            onClick={() =>
              formData.category && setShowScopeDropdown(!showScopeDropdown)
            }
          ...
          >
            ...
            >
              {formData.scope
                ? options.scopes.find((s) => s.value === formData.scope)
                    ?.text || formData.scope
                : !formData.category
                ? "請先選擇消費類別"
                : "請選擇消費種類"}
        ...
        >
          ...
          <p style={{ margin: 0 }}>• 回饋計算可能因各種條件限制而有差異</p>
        </div>
      </div>
    </div>
  );
